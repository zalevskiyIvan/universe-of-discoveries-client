...
const addPostReducer = (state = initianState, action:actionType):stateType => {
    switch(action.type){ 
        case 'GET_EVENTS':
            return{
                ...state, events: action.part.reverse()
            }
        case 'DELETE_EVENTS':
            return{
                ...state, events: [...state.events.filter((item) => item._id !== action.id)]
            }
        case 'GET_TASK':
            return{
                ...state,  tasks: action.part.reverse()
            }
        case 'DELETE_TASK':
            return{
                ...state, tasks: [...state.tasks.filter((item) => item._id !== action.id)]
            }
        case 'GET_SHORT_PROJECT':
            return{
                ...state, shortProjects: action.part.reverse()
            }
        case 'DELETE_PROJECT':
            return{
                ...state, shortProjects: [...state.shortProjects.filter((item) => item._id !== action.id)]
            }
        case 'GET_FULL_PROJECT':
            return{
                ...state, projects: action.part
            }
        case 'GET_USEFUL_LINK':
            return{
                ...state, userfulLinks: action.part.reverse()
            }
...
export const actions = {
    getEventsAC: (eventArr: Array<arrType>) => ({type: 'GET_EVENTS', part:eventArr} as const),
    //
    deleteEventsAC: (id: string) => ({type: 'DELETE_EVENTS', id} as const),
    //
    getTaskAC: (taskArr: Array<arrType>) => ({type: 'GET_TASK', part:taskArr} as const),
    //
    deleteTaskAC: (id: string) => ({type: 'DELETE_TASK', id} as const),
    //
    getShortProjectAC: (shortProjectArr: Array<arrType>) => ({type: 'GET_SHORT_PROJECT', part: shortProjectArr} as const),
    //
    deleteProjectAC: (id: string) => ({type: 'DELETE_PROJECT', id} as const),
    //
    getFullProjectAC: (projectArr: Array<any>) => ({type: 'GET_FULL_PROJECT', part: projectArr} as const),
    //
    getUsefulLinkAC: (linksArr:Array<userfulLinksType>) => ({type: 'GET_USEFUL_LINK', part:linksArr} as const)
}



export const addEventT = (klass:any, header:string, body:string, img:string) => {
...
export const addTaskT = (klass:any, header:string, body:string, img:string) => {
    const date = new Date().toLocaleDateString()
    const subject = localStorage.subject
    return (dispatch:any) => {
        const data = {klass, header, body, img, subject, date}
